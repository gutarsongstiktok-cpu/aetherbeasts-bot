...
async def api_daily(request: Request):
    ...
    with get_db() as db:
        ...
        if row["daily_date"]==today:
            return {"reward":0,"streak":row["daily_streak"],"claimed":True}
...
async def api_quests(request: Request):
    ...
    rows=[
        {"code":"summon","title":"Summon 3 beasts","progress":min(3,int(p.get("quest_summons",0) or 0)),"target":3,"reward":100},
        {"code":"battle","title":"Win 2 battles","progress":min(2,int(p.get("quest_wins",0) or 0)),"target":2,"reward":150},
        {"code":"merge","title":"Complete 1 evolution","progress":min(1,int(p.get("quest_merges",0) or 0)),"target":1,"reward":200},
    ]
    prefix=f"daily_quest_{today}_"
    claimed={r["code"][len(prefix):] for r in db_all("SELECT code FROM achievements WHERE user_id=%s AND code LIKE %s",(uid,prefix+"%")) if r["code"].startswith(prefix)}
    return {"date":str(today),"quests":[{**x,"done":x["progress"]>=x["target"],"claimed":x["code"] in claimed} for x in rows]}
...
async def api_quest_claim(request: Request):
    ...
    if p.get("quest_date") != today:
        with get_db() as db:
            db.execute("UPDATE players SET quest_date=%s,quest_summons=0,quest_wins=0,quest_merges=0 WHERE user_id=%s",(today,uid))
        p=ensure_player(uid,user.get("username"))
    progress={"summon":int(p.get("quest_summons",0) or 0),"battle":int(p.get("quest_wins",0) or 0),"merge":int(p.get("quest_merges",0) or 0)}[code]
    if progress < targets[code]: raise HTTPException(400,"Quest not completed")
    claim_code=f"daily_quest_{today}_{code}"
    with get_db() as db:
        exists=db.execute("SELECT 1 FROM achievements WHERE user_id=%s AND code=%s",(uid,claim_code)).fetchone()
        if exists: raise HTTPException(400,"Quest already claimed")
        db.execute("INSERT INTO achievements(user_id,code) VALUES(%s,%s)",(uid,claim_code))
...
async def api_battle(request: Request):
    ...
    # Short, explicit cooldown. It prevents double-click abuse but does not
    # make the Arena feel stuck for a long time.
    ...
    if last:
        ...
        if elapsed < BATTLE_COOLDOWN_SECONDS:
            remaining = max(1, int(BATTLE_COOLDOWN_SECONDS - elapsed))
            raise HTTPException(status_code=429, detail=f"Arena cooldown: {remaining}s")

    rows = db_all(
        "SELECT power FROM pets WHERE user_id=%s ORDER BY power DESC LIMIT 3",
        (uid,),
    )
    player_power = sum(int(x["power"] or 0) for x in rows)
    if player_power <= 0:
        raise HTTPException(status_code=400, detail="You need at least one Beast to fight")

    enemy = random.randint(
        max(20, player_power // 2),
        max(35, player_power + 60),
    )
    won = (player_power * random.uniform(0.85, 1.15)) >= enemy
    reward = random.randint(35, 80) if won else 10
    xp = 50 if won else 15
    today = today_utc()

    with get_db() as db:
        # Keep all battle state changes in one transaction.
        row = db.execute(
        ...
        quest_wins = int(row["quest_wins"] or 0)
        if row["quest_date"] != today:
            quest_wins = 1 if won else 0
        elif won:
            quest_wins += 1
        ...
        db.execute(
            """INSERT INTO battle_logs
               ...
               VALUES(%s,%s,%s,%s,%s)""",
            (uid, won, player_power, enemy, reward),
        )
