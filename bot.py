...
def init_db():
    ...
    statements = [
        ...
        CREATE TABLE IF NOT EXISTS daily_quest_claims (
            ...
            code TEXT NOT NULL,
            claimed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
            PRIMARY KEY(user_id, quest_date, code)
        )
        """
                """
    ...
    with get_db() as db:
        ...
        player_cols = {
            ...
            "energy": "INTEGER NOT NULL DEFAULT 100", "last_energy_at": "TIMESTAMPTZ",
        }
        ...
        # Repair legacy tables that may predate the expanded Mini App.
        ach_existing = {r["column_name"] for r in db.execute("""
            SELECT column_name FROM information_schema.columns WHERE table_name='achievements'
        """).fetchall()}
        if "claimed_at" not in ach_existing:
            db.execute("ALTER TABLE achievements ADD COLUMN claimed_at TIMESTAMPTZ NOT NULL DEFAULT NOW()")

        battle_existing = {r["column_name"] for r in db.execute("""
            SELECT column_name FROM information_schema.columns WHERE table_name='battle_logs'
        """).fetchall()}
        battle_cols = {
        ...
        for col, typ in battle_cols.items():
            if col not in battle_existing:
                db.execute(f"ALTER TABLE battle_logs ADD COLUMN {col} {typ}")
...
async def api_daily(request: Request):
    ...
    with get_db() as db:
        row = db.execute(
            "SELECT daily_date,daily_streak FROM players WHERE user_id=%s FOR UPDATE",
            (uid,),
        ).fetchone()
        if row and row["daily_date"] == today:
            return {"reward":0, "streak":int(row["daily_streak"] or 0), "claimed":True}

        old_date = row["daily_date"] if row else None
        old_streak = int(row["daily_streak"] or 0) if row else 0
        streak = old_streak + 1 if old_date and (today-old_date).days == 1 else 1
        reward = min(500, 100 + streak*25)

        db.execute(
            """UPDATE players
               SET aether=COALESCE(aether,0)+%s,
                   daily_date=%s,
                   daily_streak=%s,
                   quest_date=%s,
                   quest_summons=0,
                   ...
                   quest_merges=0
               WHERE user_id=%s""",
            (reward, today, streak, today, uid),
        )

    return {"reward":reward, "streak":streak, "claimed":False}
...
async def api_quests(request: Request):
    ...
    # Initialize today's counters once, without touching unrelated tables.
    if p.get("quest_date") != today:
        with get_db() as db:
            db.execute(
                """UPDATE players
                   SET quest_date=%s, quest_summons=0, quest_wins=0, quest_merges=0
                   WHERE user_id=%s""",
                (today, uid),
            )
        p = ensure_player(uid, user.get("username"))
    ...
    rows = [
        {"code":"summon","title":"Summon 3 beasts","progress":min(3,int(p.get("quest_summons") or 0)),"target":3,"reward":100},
        {"code":"battle","title":"Win 2 battles","progress":min(2,int(p.get("quest_wins") or 0)),"target":2,"reward":150},
        {"code":"merge","title":"Complete 1 evolution","progress":min(1,int(p.get("quest_merges") or 0)),"target":1,"reward":200},
    ]

    claimed_rows = db_all(
        "SELECT code FROM daily_quest_claims WHERE user_id=%s AND quest_date=%s",
        (uid, today),
    )
    claimed = {r["code"] for r in claimed_rows}

    return {
        "date": str(today),
        "quests": [
            {**q, "done": q["progress"] >= q["target"], "claimed": q["code"] in claimed}
            for q in rows
        ],
    }
...
async def api_quest_claim(request: Request):
    ...
    rewards = {"summon":100, "battle":150, "merge":200}
    targets = {"summon":3, "battle":2, "merge":1}
    fields = {"summon":"quest_summons", "battle":"quest_wins", "merge":"quest_merges"}
    ...
    progress = int(p.get(fields[code]) or 0)
    if progress < targets[code]:
        raise HTTPException(400, "Quest not completed")

    with get_db() as db:
        exists = db.execute(
            "SELECT 1 FROM daily_quest_claims WHERE user_id=%s AND quest_date=%s AND code=%s",
            (uid, today, code),
        ).fetchone()
        if exists:
            raise HTTPException(400, "Quest already claimed")

        db.execute(
            "INSERT INTO daily_quest_claims(user_id,quest_date,code) VALUES(%s,%s,%s)",
            (uid, today, code),
        )
        db.execute(
            "UPDATE players SET aether=COALESCE(aether,0)+%s WHERE user_id=%s",
            (rewards[code], uid),
        )
...
async def api_battle(request: Request):
    ...
    pet = db_one(
        "SELECT power,attack,defense FROM pets WHERE user_id=%s ORDER BY power DESC LIMIT 1",
        (uid,),
    )
    if not pet:
        raise HTTPException(400, "You need at least one Beast to fight")
    ...
    if last:
        ...
        if elapsed < BATTLE_COOLDOWN_SECONDS:
            raise HTTPException(429, f"Arena cooldown: {max(1,int(BATTLE_COOLDOWN_SECONDS-elapsed))}s")

    player_power = int(pet["power"] or 0) + int(pet["attack"] or 0)//5 + int(pet["defense"] or 0)//10
    enemy_power = random.randint(max(20, player_power-25), max(45, player_power+35))
    won = player_power + random.randint(-10,10) >= enemy_power
    reward = random.randint(35,80) if won else random.randint(10,25)
    xp = XP_BATTLE_WIN if won else XP_BATTLE_LOSS

    with get_db() as db:
        row = db.execute(
            "SELECT quest_date,quest_wins FROM players WHERE user_id=%s FOR UPDATE",
            (uid,),
        ).fetchone()
        if not row:
            raise HTTPException(404, "Player not found")

        qwins = int(row["quest_wins"] or 0)
        if row["quest_date"] != today_utc():
            qwins = 1 if won else 0
        elif won:
            qwins += 1
        ...
        db.execute(
            """UPDATE players
               SET aether=COALESCE(aether,0)+%s,
                   ...
                   quest_wins=%s
               WHERE user_id=%s""",
            (reward, 1 if won else 0, 0 if won else 1, today_utc(), qwins, uid),
        )
        db.execute(
            """INSERT INTO battle_logs
               (user_id,won,player_power,enemy_power,reward)
               VALUES(%s,%s,%s,%s,%s)""",
            (uid, won, player_power, enemy_power, reward),
        )

    add_player_xp(uid, xp)
